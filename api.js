...
class ModAPI {
    ...
    async fetchCurseForgeData(projectId) {
        ...
        return this.queueRequest(async () => {
            try {
                // Note: CurseForge API requires an API key for official access
                // For demo purposes, we'll use a CORS proxy or fallback data
                const proxyUrl = `https://api.allorigins.win/get?url=${encodeURIComponent(`https://www.curseforge.com/api/v1/mods/${projectId}`)}`;
                ...
                const modData = JSON.parse(data.contents);
                
                const result = {
                    description: modData.data?.summary || 'No description available',
                    downloadCount: modData.data?.downloadCount || 0,
                    dateModified: modData.data?.dateModified || null,
                    categories: modData.data?.categories?.map(cat => cat.name) || [],
                    screenshots: modData.data?.screenshots?.map(ss => ss.url) || [],
                    gameVersions: modData.data?.gameVersions || [],
                    platform: 'curseforge'
                };
                ...
                console.warn(`Failed to fetch CurseForge data for ${projectId}:`, error);
    ...
    async fetchModData(mod) {
        ...
        try {
            switch (platform) {
                case 'curseforge':
                    return await this.fetchCurseForgeData(projectId);
                case 'modrinth':
...
class DataProcessor {
    static processModList(mods) {
        return mods.map(mod => ({
            ...
            searchText: `${mod.name} ${mod.authors?.join(' ') || ''} ${mod.version}`.toLowerCase()
        }));
    }
    ...
    static filterMods(mods, searchTerm, platformFilter) {
        return mods.filter(mod => {
            const matchesSearch = !searchTerm || 
                mod.searchText.includes(searchTerm.toLowerCase()) ||
